...
class InterviewService {
  ...
  Function(Uint8List)? _onAudioReceived; // Add audio callback

  // Store current streaming message part
  String _currentStreamingMessage = '';
  String _currentMessageId = '';
  Uint8List? _currentAudioData; // Store current audio data
  ...
  Future<InterviewPrepareResponse> submitInterviewPreparation(
    InterviewPrepareRequest request,
    String idToken,
  ) async {
    try {
      ...
      if (request.linkedinLink != null)
        multipart.fields['linkedin_link'] = request.linkedinLink!;
      if (request.githubLink != null)
        multipart.fields['github_link'] = request.githubLink!;
      if (request.portfolioLink != null)
        multipart.fields['portfolio_link'] = request.portfolioLink!;
      if (request.additionalInfo != null)
        multipart.fields['additional_info'] = request.additionalInfo!;
      if (request.numQuestions != null)
        multipart.fields['num_questions'] = request.numQuestions.toString();
      if (request.sessionId != null)
        multipart.fields['session_id'] = request.sessionId!;
      ...
      if (request.resumeFile.bytes != null) {
        ...
        multipart.files.add(
          await http.MultipartFile.fromPath('file', request.resumeFile.path!),
        );
      } else {
    ...
    try {
      ...
      try {
        ...
        final mockSession = {
          ...
          'websocket_parameter':
              'token=mock_ws_token_${DateTime.now().millisecondsSinceEpoch}',
        };
        ...
        throw Exception(
          'Failed to start interview session: Real API failed ($e), Mock data also failed ($mockError)',
        );
      }
    }
  }
  ...
  Future<Map<String, dynamic>> getInterviewFeedback(
    ...
    try {
      ...
      debugPrint(
        'Calling POST /interviews/$workflowId/$sessionId/feedback API...',
      );

      final response = await http.post(
        Uri.parse(
          '${ApiConfig.baseUrl}${ApiConfig.interviewFeedbackEndpoint(workflowId, sessionId)}',
        ),
        headers: {
          'Content-Type': 'application/json',
          'Authorization': 'Bearer $token',
      ...
      try {
        ...
        debugPrint('❌ Mock data also failed: $mockError');
        throw Exception(
          'Failed to get interview feedback: Real API failed ($e), Mock data also failed ($mockError)',
        );
      }
    }
  }
  ...
  Future<List<Map<String, dynamic>>> getInterviewHistory(
    String workflowId,
  ) async {
    try {
      final token = await _getIdToken();
      ...
      final apiUrl =
          '${ApiConfig.baseUrl}${ApiConfig.workflowInterviewsEndpoint(workflowId)}';
      ...
      if (response.statusCode == 200) {
        ...
        debugPrint(
          '✅ Real API: Interview history loaded successfully (${interviews.length} items)',
        );
        return interviews;
      } else {
        throw Exception(
      ...
      try {
        ...
        final filteredHistory = MockData.interviewHistory
            ...
            .toList();
        debugPrint(
          '✅ Mock API: Interview history loaded with mock data (${filteredHistory.length} items)',
        );
        return filteredHistory;
      } catch (mockError) {
        debugPrint('❌ Mock data also failed: $mockError');
        throw Exception(
          'Failed to get interview history: Real API failed ($e), Mock data also failed ($mockError)',
        );
      }
    }
  }
          ...
          try {
            ...
            if (data.containsKey('mime_type') &&
              ...
              if (_currentMessageId.isEmpty) {
                _currentMessageId = DateTime.now().millisecondsSinceEpoch
                    .toString();
              }
              return;
            }
            ...
            if (data.containsKey('mime_type') &&
              ...
              final audioDataBase64 = data['data'] ?? '';
              if (audioDataBase64.isNotEmpty) {
                try {
                  _currentAudioData = base64Decode(audioDataBase64);
                  debugPrint(
                    '🎵 Audio data received: ${_currentAudioData!.length} bytes',
                  );
                } catch (e) {
            ...
            // Handle turn completion - send both text and audio if available
            if (data.containsKey('turn_complete') && data['turn_complete'] == true) {
              if (_currentStreamingMessage.isNotEmpty || _currentAudioData != null) {
                String messageContent;
                
                // Determine message content based on what we received
                if (_currentStreamingMessage.isNotEmpty && _currentAudioData != null) {
                  // Both text and audio available
                  messageContent = _currentStreamingMessage;
                } else if (_currentStreamingMessage.isNotEmpty) {
                  // Only text available
                  messageContent = _currentStreamingMessage;
                } else if (_currentAudioData != null) {
                  // Only audio available - you might want to transcribe or show a better message
                  messageContent = '🎵 Audio Response (Click to play)';
                } else {
                  messageContent = '[Empty Message]';
                }
                
                final chatMessage = ChatMessage(
                  id: _currentMessageId.isEmpty
                ...
                // Also trigger audio callback if available
                if (_currentAudioData != null && _onAudioReceived != null) {
                  _onAudioReceived!(_currentAudioData!);
                }
            ...
            if (data.containsKey('messageId') || data.containsKey('role')) {
              final chatMessage = ChatMessage(
                ...
                timestamp:
                    DateTime.tryParse(data['createAt'] ?? '') ?? DateTime.now(),
              );
              _onMessageReceived?.call(chatMessage);
            }
          } catch (e) {
  ...
  void sendMessage(String message) {
    if (_wsConnected && _channel != null) {
      ...
      final messageData = {'mime_type': 'text/plain', 'data': message};
  ...
  // set WebSocket event listeners - updated to include audio callback
  void setWebSocketListeners({
    Function(ChatMessage)? onMessageReceived,
    Function()? onDisconnected,
    Function(String)? onError,
    Function(Uint8List)? onAudioReceived, // Add audio callback parameter
  }) {
    _onMessageReceived = onMessageReceived;
    _onDisconnected = onDisconnected;
    _onError = onError;
    _onAudioReceived = onAudioReceived; // Set audio callback
  }
